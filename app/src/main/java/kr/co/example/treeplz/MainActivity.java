...
//    private MaterialSwitch switchLanguage;
    ...
    private void setupListeners() {
        ...
        btnViewUsage.setOnClickListener(v -> {
            ...
            // 보낼 객체 준비 (model.AiUsage 사용)
            AiUsage usageToSend;

            if (latestUsageFromApi != null) {
                // 서버 데이터가 있으면 그거 그대로 사용
                usageToSend = latestUsageFromApi;
            } else {
                // 없으면 로컬 변수들로 새로 만들기
                // (주의: 생성자 순서와 타입을 model/AiUsage.java와 똑같이 맞춰야 함)
                usageToSend = new AiUsage(
                        requests,
                        (int) tokens,
                        (double) timeSpentMs, // 타입 맞추기 (double 등)
                        carbonFootprint
                );
            }

            Intent intent = new Intent(MainActivity.this, UsageDetailsActivity.class);
            intent.putExtra("extra_data", usageToSend); // 택배 싣기
            startActivity(intent);
        });
